...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // ── Manual Refresh Handler (user-triggered data re-fetch) ─────────────────
  useEffect(() => {
    // Check if manual refresh was requested via HYDRATE_TABLES action
    // We use a flag approach: look for a pending refresh request
    let cancelled = false;

    // Store this effect for accessing in the manual refresh handler
    const handleManualRefresh = async () => {
      if (isHydrating.current) {
        console.log('[handleManualRefresh] Already hydrating, skipping...');
        return;
      }
      ...
      try {
        ...
        // Re-fetch only the critical table data (Phase 1 equivalent)
        const [
          tanksRaw, pumpsRaw, nozzlesRaw, tracksRaw, driversRaw,
          productsRaw, brandsRaw,
          pompistesRaw, chefsRaw, gerantsRaw, magasinRaw,
        ...
        const drivers = (driversRaw as any[]).map(mapDriver);
        const products = (productsRaw as any[]).map(mapProduct);
        const productBrands = (brandsRaw as any[]).map(mapBrand);
        const brigades = (brigadesRaw as any[]).map(b => mapBrigade(b, (brigadeAssignmentsRaw as any[]).filter(a => a.brigade_id === b.id).map(a => a.pompiste_id)));
        const settings = mapSettings(settingsRaw as any);
        const pompistesBase = (pompistesRaw as any[]).map(mapPompiste);
        const brigadeChiefsBase = (chefsRaw as any[]).map(mapBrigadeChef);
        const gerantsBase = (gerantsRaw as any[]).map(mapGerant);
        const magasinWorkersBase = (magasinRaw as any[]).map(mapMagasinWorker);
        const clientsBase = (clientsRaw as any[]).map(mapClient);
        const suppliersBase = (suppliersRaw as any[]).map(mapSupplier);
        const permissionTemplates = (permissionTemplatesRaw as any[]).map(mapPermissionTemplate);
        ...
        dispatch({
          ...
          payload: {
            ...
            brigades,
            settings,
            pompistes: pompistesBase,
            brigadeChefs: brigadeChiefsBase,
            gerants: gerantsBase,
            magasinWorkers: magasinWorkersBase,
            clients: clientsBase,
            suppliers: suppliersBase,
            permissionTemplates,
          },
        });

        console.log('[handleManualRefresh] Manual data refresh complete');
        ...
        console.error('[handleManualRefresh] Failed:', err);
        if (!cancelled) {
          dispatch({ type: 'SET_LOADING', payload: false });
          dispatch({
            type: 'ADD_TOAST',
            payload: {
              type: 'error',
              message: `Erreur lors de l'actualisation: ${err instanceof Error ? err.message : 'Erreur inconnue'}`,
            },
          });
        }
      } finally {
    ...
    // Store the handler so it can be called from actions
    (window as any).__refreshAppData = handleManualRefresh;

    return () => {
      cancelled = true;
      delete (window as any).__refreshAppData;
    };
  }, [dispatch]);
