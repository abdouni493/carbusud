...
import {
  ...
  X, Clock, Package, Droplets, ArrowUpRight, ArrowDownRight,
  Zap, Users, Building2, ChevronRight, Calendar
} from "lucide-react";
...
const Dashboard = () => {
  ...
  const {
    ...
    pompistes, pumps, expenses, deliveryNotes, brigadeChefs, suppliers,
    gerants, magasinWorkers, currentUserRole, settings, fuelInvoices
  } = useAppState();
  ...
  const todayExpensesArr = useMemo(() => expenses.filter(e => e.date.startsWith(todayStr)), [expenses, todayStr]);

  const fuelRevenue = useMemo(() => todayFuelSales.reduce((sum, s) => sum + s.total, 0), [todayFuelSales]);
  const shopRevenue = useMemo(() => todayShopSales.reduce((sum, s) => sum + s.total, 0), [todayShopSales]);
  const todayLiters = useMemo(() => todayFuelSales.reduce((sum, s) => sum + s.liters, 0), [todayFuelSales]);
  const totalExpense = useMemo(() => todayExpensesArr.reduce((sum, e) => sum + e.amount, 0), [todayExpensesArr]);
  const netRevenue = fuelRevenue + shopRevenue - totalExpense;

  const pendingAcomptesCount = useMemo(() => {
    const allWorkers = [...(pompistes || []), ...(brigadeChefs || []), ...(gerants || []), ...(magasinWorkers || [])];
    return allWorkers.reduce((count, worker) => count + (worker.acomptes?.filter(a => !a.isPaid).length || 0), 0);
  ...
  const activeBrigadeFuelSales = useMemo(
    () => fuelSales.filter(sale => activeBrigade && sale.brigadeId === activeBrigade.id),
    [fuelSales, activeBrigade]
  );
  const activeBrigadeLiters = useMemo(() => activeBrigadeFuelSales.reduce((sum, sale) => sum + sale.liters, 0), [activeBrigadeFuelSales]);
  const activeBrigadeCollected = useMemo(() => activeBrigadeFuelSales.reduce((sum, sale) => sum + sale.total, 0), [activeBrigadeFuelSales]);
  ...
  const elapsedTime = useMemo(() => {
    if (!activeBrigade?.startTimestamp) return "00:00:00";
    const diff = now.getTime() - new Date(activeBrigade.startTimestamp).getTime();
  ...
  const recentActivity = useMemo(() => [
    ...fuelSales.map(s => ({ date: s.date, amount: s.total, label: "Vente Carburant", type: "fuel" })),
    ...shopSales.map(s => ({ date: s.date, amount: s.total, label: "Vente Magasin",   type: "shop" })),
    ...expenses.map(e => ({ date: e.date,  amount: e.amount, label: "Dépense",         type: "expense" })),
  ].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 6),
  [fuelSales, shopSales, expenses]);

  const timeAgo = (ds: string) => {
    const mins = Math.floor((now.getTime() - new Date(ds).getTime()) / 60000);
      ...
      >
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            {/* Brigade Active Badge si existe */}
            {activeBrigade && (
              <div className="bg-white/10 backdrop-blur rounded-2xl p-4 border border-white/20">
                ...
                <p className="text-white/50 text-xs mt-0.5">Depuis {activeBrigade.startTimestamp ? new Date(activeBrigade.startTimestamp).toLocaleTimeString('fr-DZ', {hour:'2-digit', minute:'2-digit'}) : '--:--'}</p>
              </div>
            )}
            
            <button onClick={() => navigate("/fuel-sales")}
              className="flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-sm text-[#001f5c] transition-all hover:shadow-lg h-full"
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl border bg-white shadow-sm flex flex-col justify-center relative overflow-hidden">
            <div className="absolute -right-6 -bottom-6 opacity-[0.03] text-blue-600">
              <Droplets className="w-32 h-32" />
            </div>
            <p className="text-xs font-bold text-slate-500 uppercase mb-1 tracking-wider">Carburant</p>
            <p className="text-2xl font-black text-blue-700 relative z-10">{fuelRevenue.toLocaleString('fr-DZ')} <span className="text-sm">DA</span></p>
          </div>
          <div className="p-5 rounded-2xl border bg-white shadow-sm flex flex-col justify-center relative overflow-hidden">
            <div className="absolute -right-6 -bottom-6 opacity-[0.03] text-purple-600">
              <ShoppingCart className="w-32 h-32" />
            </div>
            <p className="text-xs font-bold text-slate-500 uppercase mb-1 tracking-wider">Magasin</p>
          ...
          <div className="p-5 rounded-2xl border bg-white shadow-sm flex flex-col justify-center relative overflow-hidden">
            <div className="absolute -right-6 -bottom-6 opacity-[0.03] text-red-600">
              <TrendingUp className="w-32 h-32" />
            </div>
            <p className="text-xs font-bold text-slate-500 uppercase mb-1 tracking-wider">Dépenses</p>
            <p className="text-2xl font-black text-red-700 relative z-10">{totalExpense.toLocaleString('fr-DZ')} <span className="text-sm">DA</span></p>
          </div>
          <div className="p-5 rounded-2xl border bg-white shadow-sm flex flex-col justify-center relative overflow-hidden">
            <div className="absolute -right-6 -bottom-6 opacity-[0.03] text-green-600">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            <div style={{ height: 200 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                  ...
                  <Tooltip
                    contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 8px 32px rgba(0,48,135,0.12)", fontSize: 12, fontWeight: 700 }}
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
            ...
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(255,184,0,0.15)" }}>
                  <Fuel className="w-4.5 h-4.5" style={{ color: "#FFB800" }} />
                </div>
                <div>
            ...
            <div className="space-y-4">
              ...
              ) : tanks.map(tank => <TankBar key={tank.id} tank={tank} />)}
            </div>
          </motion.div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}
            className="card-naftal p-5">
            <div className="flex items-center gap-2 mb-4">
              <Activity className="w-4 h-4" style={{ color: "var(--naftal-blue-600)" }} />
              <h3 className="text-xs font-black uppercase tracking-wider" style={{ color: "var(--naftal-blue-600)" }}>
                Brigade Active
              </h3>
              {activeBrigade && (
            ...
            {activeBrigade ? (
              ...
              <div className="text-center py-8">
                ...
                <button onClick={() => navigate("/brigades")} className="btn-primary text-xs px-4 py-2">
                  Démarrer une Brigade
                </button>
              </div>
            )}
          </motion.div>

          {/* Prochains RDV Paiements */}
          <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.35 }}
            ...
            <div className="space-y-3">
              ...
              ) : upcomingPaymentsList.map((app, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl border border-slate-100 bg-white/50">
                    <div className={cn(
                      ...
                      isLate ? "bg-red-500 shadow-md shadow-red-500/20" : diff <= 3 ? "bg-orange-500" : "bg-blue-500"
                    )}>
                    ...
                    <span className="text-xs font-black tabular-nums whitespace-nowrap">
                      {app.amount.toLocaleString("fr-DZ")} DA
                    </span>
                  </div>
                );
              })}
          ...
          <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
            className="card-glass p-5">
            <h3 className="text-xs font-black uppercase tracking-wider mb-4" style={{ color: "var(--naftal-blue-600)" }}>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 transition-colors cursor-default">
                  ...
                  <span className="text-sm text-slate-600 flex-1">{stat.label}</span>
                  <span className="text-sm font-black" style={{ color: "var(--naftal-blue-700)" }}>{stat.value}</span>
                </div>
              ))}
            </div>
          </motion.div>
